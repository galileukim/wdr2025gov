...
gsps_national <- gsps_standards |>
  filter(
    group == "All"
  )

gsps_region <- gsps_standards |>
  filter(
    group == "Region"
  )

gsps_institution <- gsps_standards |>
  filter(
    group == "Institution"
  )

usethis::use_data(gsps_national, overwrite = TRUE)
usethis::use_data(gsps_region, overwrite = TRUE)
usethis::use_data(gsps_institution, overwrite = TRUE)
